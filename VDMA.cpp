...
int vdma_test() {
	...
	while(1){
		for (j = 0; j < 9; j++)
			handle.fb1VirtualAddress[j] = c++;
	        sleep(1);
		//usleep(500000);
		printf("\n");
		for (j = 0; j < 9; j++)
			printf(" %02x", handle.fb2VirtualAddress[j]); printf("\t");
	}
...
int vdma_mat_test(){
   ...
   vdma_handle handle;
    // Setup VDMA handle and memory-mapped ranges
    vdma_setup(&handle, 0x43000000, frame_width, frame_height, 3, 0x1e000000, 0x1e100000, 0x1e200000);
    // Start triple buffering
    vdma_start_triple_buffering(&handle);

   Mat in(frame_height,frame_width,CV_8UC3,handle.fb1VirtualAddress);
   Mat out(frame_height,frame_width,CV_8UC3,handle.fb2VirtualAddress);
